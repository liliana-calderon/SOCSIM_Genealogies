...
## Modified function to calculate the life table for each simulation

lt_socsim_sims <- function(asmr_socsim_sims){ 

  lt_socsim_sims <- asmr_socsim_sims %>% 
  mutate(Age = as.numeric(str_extract(age, "\\d+"))) %>% 
  rename(mx = socsim) %>% 
